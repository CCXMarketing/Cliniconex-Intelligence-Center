export default {
  ...
  _renderAIGrid(containerEl, data) {
    ...
    grid.innerHTML = [
      this._kpiCard('AI Products Launched', `${k.ai_products_launched.value} of ${k.ai_products_launched.target_per_quarter} this quarter`, null, 'yellow', 'Quarterly'),
      this._kpiCard('AI Skills Pilots', `${k.ai_skills_pilots.value} of ${k.ai_skills_pilots.target_annual} annual`, null, 'yellow', 'Quarterly',
        this._progressBar(k.ai_skills_pilots.value, k.ai_skills_pilots.target_annual, 'yellow')),
      this._kpiCard('Customer Validations', `${k.customer_validations.value} of ${k.customer_validations.target} by Q3`, null, 'yellow', 'Quarterly'),
      this._kpiCard('AI-Specific Revenue MRR', CIC.formatCurrency(k.ai_specific_revenue.value),
        `Target: ${CIC.formatCurrency(k.ai_specific_revenue.monthly_target)}/mo`, 'yellow', 'Monthly'),
      this._kpiCard('AI Case Studies', `${k.ai_case_studies.value} of ${k.ai_case_studies.target} annual`, null, 'red', 'Quarterly')
    ].join('');
  },
  ...
  _renderDeliveryGrid(containerEl, data) {
    ...
    grid.innerHTML = [
      this._kpiCard('Say/Do Ratio', k.say_do_ratio.value + '%',
        `Target: ${k.say_do_ratio.target}%`, 'yellow', 'Quarterly'),
      this._kpiCard('Customer-Facing Bug Reduction', k.bug_reduction.value + '%',
        `Target: ${k.bug_reduction.target}% QoQ`, 'red', 'Quarterly'),
      this._kpiCard('Strategic Allocation', k.strategic_allocation.value + '%',
        `Target: ${k.strategic_allocation.target}%`, 'red', 'Quarterly')
    ].join('');
  },
  ...
  _kpiCard(label, value, target, status, cadence, extraHtml) {
    return `
